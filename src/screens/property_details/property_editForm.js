...
const Component = (props) => {
    ...
    const OnSubmit = async () => {
        let rslt, data, changes = [], propertyId, property;

        property = row['property'];
        propertyId = row['property'].find((x) => x.type === 'keyid').value;
            ...
            if (changes.length > 0) {
                ...
                if (tmp.length > 0) {
                    ...
                    if (rslt.status) {
                        ...
                        if (Helper.IsNullValue(mapItem.navpropname)) propertyId = rslt.id;
        ...
        global.AlertPopup("success", "Property is updated successfully!");
        setShowUpdate(false);
        NavigateTo(-1);
    }
