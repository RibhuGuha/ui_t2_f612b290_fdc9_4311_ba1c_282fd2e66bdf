...
const Component = (props) => {
    ...
    const FetchResults = async () => {
        ...
        await Api.GetPropertiesCount(query)
            .then(async (res) => {
                if (res.status) {
                ...
                if (res.status) {
                    ...
                    for (let i = 0; i < _rows.length; i++) {
                        _rows[i].id = Helper.GetGUID();
                                                                                                _rows[i].MainImage &&
                            await Api.GetDocumentSingleMedia(_rows[i].MainImage.DocId, true, "image/jpeg").then((rslt) => {
                                _rows[i].PropertyMainImageData = rslt.values;
                            })
                                            }
                } else {
                    console.log(res.statusText);
    ...
    const OnViewChanged = (e) => {
        let _route;
        if (e === 'DETAILS') _route = `/Properties`;
        if (e === 'TILES') _route = `/Propertytiles`;
        if (e === 'CONTENT') _route = `/Propertycontent`;
        if (e === 'LIST') _route = `/Propertylist`;
    ...
    const OnCloseClicked = async (e) => {
        if (e) {
            const rslt = await Api.SetPropertySingle({ PropId: deletedId, Deleted: true });
            if (rslt.status) {
                setInitialize(true);
    ...
    const OnActionClicked = (id, type) => {
        let _route;
        if (type === 'edit') _route = `/Properties/edit/${id}`;
        if (type === 'view') _route = `/Properties/view/${id}`;
    ...
    return (
        ...
        <>
            <Container {...props}>
                ...
                <Box style={{ width: '100%' }}>
                    <DataGrid keyId={'PropId'} rowsCount={rowsCount} rows={rows} sortBy={sortBy} pageInfo={pageInfo} onActionClicked={OnActionClicked}
